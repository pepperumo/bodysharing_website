  ...
  return corsHandler(request, response, async () => {
    try {
      ...
      if (!to || !from || !subject || !html) {
        return response.status(400).json({ error: 'Missing required fields: to, from, subject, html' });
      }
...
export const sendContactFormEmail = functions.https.onRequest((request, response) => {
  return corsHandler(request, response, async () => {
    try {
      // Validate request method
      if (request.method !== 'POST') {
      ...
      // Validate the required fields
      if (!name || !email || !inquiryType || !message || consent === undefined) {
        return response.status(400).json({ error: 'Missing required form fields' });
      }

      // Initialize Resend with API key
      const resend = new Resend(functions.config().resend.api_key);
      const fromEmail = functions.config().email.from || 'contact@bodysharing.com';
      const adminEmail = functions.config().email.admin || 'admin@bodysharing.com';
